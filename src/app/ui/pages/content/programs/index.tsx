...
export default function ProgramPage() {
    ...
    const {data, refetch, isLoading, isFetching} = useQuery({queryKey: ['programs', filter], queryFn: () => api.get(filter), refetchOnWindowFocus:false}); 
	...
	const columns: ColumnsType<Program> = [
		...
		},
		{
			...
			dataIndex: "name",
			render: (_, record) => (record.thumbnail && <img width={50} height={50} src={`data:image/png;base64,${record.thumbnail}`} alt="thumbnail" />),
		},
		{
		...
		},
		{
			...
			dataIndex: "registrationEnd",
			render: (_, record) => <Button onClick={() => navigate(`/levels?programId=${record.id}`)} icon={<Iconify icon="solar:layers-minimalistic-broken" size={18} /> }>
									{t("app.levels.title")}
								</Button> ,
		},
		{
			...
			render: (_, record) => (
				<div className="flex w-full justify-end text-gray">
					...
					<Popconfirm title={t("common.confirm-launch")} okText={t("common.ok")} cancelText={t("common.cancel")} placement="left" onConfirm={() => onLaunch(record)}>
						<button type="button">
							<Iconify icon="solar:rocket-2-broken" size={18} className="text-success-light" />
						</button>
					</Popconfirm>
